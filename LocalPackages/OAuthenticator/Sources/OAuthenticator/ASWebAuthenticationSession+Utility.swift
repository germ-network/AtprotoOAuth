import Foundation
	...
	extension ASWebAuthenticationSession {
		@MainActor
		convenience init(
			url: URL, callbackURLScheme: String,
			completionHandler: @escaping (Result<URL, Error>) -> Void
		) {
			self.init(
				url: url, callbackURLScheme: callbackURLScheme,
				completionHandler: { (resultURL, error) in
	...
	extension ASWebAuthenticationSession {
			...
			{
				try await begin(
					with: url, callbackURLScheme: scheme,
					contextProvider: CredentialWindowProvider())
			}

			@MainActor
			public static func userAuthenticator(url: URL, scheme: String) async throws
				-> URL
			{

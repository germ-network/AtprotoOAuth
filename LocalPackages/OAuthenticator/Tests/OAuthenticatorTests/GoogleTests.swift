...
struct GoogleTests {
	...
	func testSuppliedParameters() async throws {
		...
		let provider: URLResponseProvider = { _ in
		...
		let googleURLProvider = try await config.tokenHandling.authorizationURLProvider(
			params)

		let urlComponent = URLComponents(
			url: googleURLProvider, resolvingAgainstBaseURL: true)
		#expect(urlComponent?.scheme == GoogleAPI.scheme)

		// Validate query items inclusion and value
		#expect(
			urlComponent!.queryItems!.contains(where: {
				$0.name == GoogleAPI.includeGrantedScopeKey
			}))
		#expect(
			urlComponent!.queryItems!.contains(where: { $0.name == GoogleAPI.loginHint }
			))

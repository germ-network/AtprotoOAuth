...
final class MockURLResponseProvider: @unchecked Sendable {
	...
	static let dummyResponse: (Data, URLResponse) = (
		...
		URLResponse(
			url: URL(string: "https://test.com")!, mimeType: nil,
			expectedContentLength: 5, textEncodingName: nil)
	)
}
...
final class AuthenticatorTests: XCTestCase {
	...
	func testInitialLogin() async throws {
		...
		await fulfillment(
			of: [retrieveTokenExp, userAuthExp, storeTokenExp, authedLoadExp],
	...
	func testExistingLogin() async throws {
		...
		await fulfillment(
			of: [retrieveTokenExp, authedLoadExp], timeout: 1.0, enforceOrder: true)
	}
	...
	func testExpiredTokenRefresh() async throws {
		...
		await fulfillment(
			of: [retrieveTokenExp, refreshExp, storeTokenExp, authedLoadExp],
			timeout: 1.0, enforceOrder: true)
	}
	...
	func testExpiredTokenRefreshFailing() async throws {
		...
		await fulfillment(
			of: [retrieveTokenExp, refreshExp, clearTokenExp], timeout: 1.0,
			enforceOrder: true)
	}
	...
	func testManualAuthentication() async throws {
		let urlProvider: TokenHandling.AuthorizationURLProvider = { parameters in
			return URL(
				string: "my://auth?client_id=\(parameters.credentials.clientId)")!
		}
	...
	func testManualAuthenticationWithSuccessResult() async throws {
		...
		let authenticationCallback: Authenticator.AuthenticationStatusHandler = {
			@MainActor result in
			switch result {
			case .failure(_):
				XCTFail()
	...
	func testUnauthorizedRequestRefreshes() async throws {
		...
		mockLoader.responses = [
			.success(
				(
					Data(),
					HTTPURLResponse(
						url: requestedURL, statusCode: 401,
			...
			.success(
				(
					mockData,
					HTTPURLResponse(
						url: requestedURL, statusCode: 200,
						httpVersion: nil, headerFields: nil)!
				)),
		]
		...
		XCTAssertEqual(
			mockLoader.requests[0].allHTTPHeaderFields!["Authorization"],
			"Bearer EXPIRED")
		XCTAssertEqual(
			mockLoader.requests[1].allHTTPHeaderFields!["Authorization"],
			"Bearer REFRESHED")
	}
	...
	func testTokenExpiredAfterUseRefresh() async throws {
		...
		let storedLogin = Login(
			accessToken: Token(
				value: "EXPIRE SOON", expiry: Date().addingTimeInterval(1)),
			refreshToken: Token(value: "REFRESH")
		)
		...
		XCTAssertEqual(
			sentRequestsOne.first?.value(forHTTPHeaderField: "Authorization"),
			"Bearer EXPIRE SOON", "Non expired token should be used for first request")
		XCTAssertTrue(
			refreshedLogins.isEmpty, "Token should not be refreshed after first request"
		)
		XCTAssertEqual(loadLoginCount, 1, "Login should be loaded from storage once")
		XCTAssertTrue(
			savedLogins.isEmpty,
			"Login storage should not be updated after first request")
		...
		XCTAssertEqual(
			...
			"Bearer REFRESHED", "Refreshed token should be used for second request")
		XCTAssertEqual(
		...
		XCTAssertEqual(
			thirdRequest?.value(forHTTPHeaderField: "Authorization"),
			"Bearer REFRESHED", "Refreshed token should be used for third request")
		XCTAssertEqual(
			savedLogins.count, 1, "No additional logins should be saved to storage")
	}
}
